...
_esamatti_dotfiles_init() {
    ...
    local pure_path="${prefix}/share/zsh/site-functions/prompt_pure_setup"
    if [ -f "$pure_path" ]; then
        pure_setup="$pure_path"
    fi
...
,oc-commit() {
    ...
    local model="${EM_COMMIT_MODEL:-github-copilot/claude-sonnet-4.6}"
    opencode --model "$model" --prompt "$message"
}
